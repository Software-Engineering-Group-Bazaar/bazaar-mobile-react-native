...
const OrderStatusEnum = [
 ...
 export default function OrdersScreen() {
  ...
  const fetchOrders = useCallback(() => {
    setRefreshing(true);
    api
      .get('Order') // <- veliko O, kao na Swaggeru
      .then((res) => {
        const mapped = res.data.map((o: any, index: number) => ({
          id: o.id ?? index,
          createdAt: o.time,
          totalAmount: o.total,
          status: OrderStatusEnum[o.status] as OrderStatus,
        }));
        setOrders(mapped);
      })
   ...
   return (
    <>
      <Stack.Screen options={{ title: t('orders'), headerTitleAlign: 'center' }} />
      <View style={{ flex: 1 }}>
