...
export default function ReviewCard({ review, onResponseSubmitted }: ReviewCardProps) {
  ...
  const handleSubmitResponse = async () => {
    ...
    setIsLoading(true);
    try {
      const updatedReview = await apiSubmitSellerResponse({
        reviewId: review.id,
        orderId: review.orderId, // ✅ ispravno prosleđivanje
        responseText
      });
      ...
      {isRespondingActive && (
        <View style={styles.respondFormSection}>
          <Text style={styles.formLabel}>
            {review.sellerResponse ? /**/('edit_your_response') : t('respond_to_this_review')}
          </Text>
          <TextInput
