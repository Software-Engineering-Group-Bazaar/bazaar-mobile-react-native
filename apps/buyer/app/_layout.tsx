...
async function getAuthTokenFromStorage(): Promise<string | null> {
  try {
    const token = await SecureStore.getItemAsync('auth_token'); // Koristi isti ključ kao u login.tsx
...
const DEVICES_API_ENDPOINT = 'http://192.168.0.25:5054/api/Devices/pushNotification';
  ...
  useEffect(() => {
    if (loaded) {
      ...
      const checkAuthAndRegisterPush = async () => {
        ...
        console.log("UserToken Notif:" + userToken);
